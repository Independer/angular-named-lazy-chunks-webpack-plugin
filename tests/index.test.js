...
describe('AngularNamedLazyChunksWebpackPlugin', () => {
  ...
  it('single app mode - default config - multiple runs - chunk names should not change', async () => {
    const fs = new MemoryFS();
    const compiler = webpack({
      entry: path.join(__dirname, 'webpack-apps/default-config/src/main.js'),
      output: { path: __dirname },
    ...
    const run1Stats = await runAsync();

    verifyRun(run1Stats);
    
    const run2Stats = await runAsync();
